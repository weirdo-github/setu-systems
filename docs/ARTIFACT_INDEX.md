# Documentation And Artifact Index

## Setu Systems Controlling Docs

- `README.md`: system overview, local setup, commands, and portal routes.
- `docs/SETU_SYSTEMS_ARCHITECTURE.md`: consolidated runtime, routing, and data architecture.
- `docs/AWS_DEPLOYMENT.md`: AWS deployment guide and production checklist.
- `docs/OPERATIONS_RUNBOOK.md`: smoke tests, recurring checks, and incident response.
- `docs/PORTAL_FUNCTIONAL_MAP.md`: active and planned portal functions.
- `docs/DATA_AND_SECURITY.md`: data boundaries, security posture, and production hardening.

## Finance Inherited Artifacts

- `docs/architecture.md`
- `docs/current-state.md`
- `docs/detailed-design.md`
- `docs/design-system.md`
- `docs/feature-list.md`
- `docs/flow-diagrams.md`
- `docs/process-flow.md`
- `docs/requirements.md`
- `docs/test-plan.md`
- `docs/user-guide.md`
- `docs/aws-solution-architecture.md`
- `files/phase1_prototype.html`
- `files/setu_phase1_requirements.html`
- `files/setu_finance_pitch_deck.html`
- `files/setu_finance_pitch_deck.pptx`

## Discovery Inherited Artifacts

- `docs/discover/PRODUCT_CAPABILITIES.md`
- `docs/discover/FUNCTIONAL_SPEC.md`
- `docs/discover/TECHNICAL_ARCHITECTURE.md`
- `docs/discover/FINANCE_ENGAGEMENT_INTEGRATION.md`
- `docs/discover/NON_FUNCTIONAL_REQUIREMENTS.md`
- `docs/discover/NON_TECHNICAL_OVERVIEW.md`
- `docs/discover/RELEASE_READINESS.md`
- `docs/discover/Discover_Product_Deck.pptx`

## Runtime Data Artifacts

- Finance seed source: `server/storage/app-state.json`
- Discover source registry: `data/source-registry.json`
- Discover schema: `db/schema.sql`
- Finance migrations: `server/db/migrations/`
